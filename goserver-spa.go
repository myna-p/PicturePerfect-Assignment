...
func (h *Handler) getAllMovies(c *gin.Context) {
	...
	h.DB.Find(&lists) //paginating: h.DB.Offset(6).Limit(6).Find(&lists)
...
func (h *Handler) getAllMovies(c *gin.Context) {
	...
	paginator := p.Paginator{DB: h.DB, Page: "1", PerPage: "6"}
	data := paginator.Paginate(&lists)
	h.DB.Find(&data)
	c.JSON(http.StatusOK, lists)
}
